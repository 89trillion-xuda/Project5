...
public class RewardChildObjects : MonoBehaviour
{
    ...
    {
        //设置段位分数
        if (RewardRankPoints != null) RewardRankPointsTxt.text = RewardRankPoints.ToString();
        //设置奖励文本
        if (Reward != null) RewardTxt.text = Reward;
        //设置奖励内容
        if (RewardContent != null) RewardContentTxt.text = RewardContent;
        //设置已领取文本状态
        if (RewardReceived != null) RewardReceivedTxt.gameObject.SetActive(RewardReceived);
        //设置可领取按钮状态
        if (RewardReceive != null) RewardReceiveBtn.gameObject.SetActive(RewardReceive);
